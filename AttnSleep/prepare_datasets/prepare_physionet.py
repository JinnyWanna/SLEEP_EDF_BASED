...
# 명령어 파싱 설정
DATA_EDF78 = "/Users/woojin/Documents/paper/Physionet_EDF_based/sleep-edf-database-expanded-1.0.0/sleep-cassette"

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--data_dir", type=str, default="DATA_EDF78",
    ...
    for i in range(len(psg_fnames)):
        ...
        # raw_ch_df = raw.to_data_frame(scaling_time=100.0)[select_ch] # scaling_time 메소드가 실행 안됨
        raw_data = raw.get_data(picks=select_ch)  # Get the raw data for the selected channel
        raw_ch_df = pd.DataFrame(raw_data.T, columns=[select_ch])  # Convert to DataFrame
        ...
        for a in ann[0]:
            ...
            if label != UNKNOWN:
                ...
                label_epoch = np.ones(duration_epoch, dtype=int) * label # np.int 메서드 더이상 사용 안함 -> np.int -> int로 수정
